(function(thisObj){
    ...
    var panelGlobal = thisObj;
        ...
        */ 
        ...
        // KEY2
        // ====
        var key2 = keyFrameRepeaterWindow.add("group", undefined, {name: "key2"}); 
        ...
        // FREQ
        // ====
        var freq = keyFrameRepeaterWindow.add("group", undefined, {name: "freq"}); 
        ...
        // ROTATIONREPEATE
        // ===============
        var holdFrame = keyFrameRepeaterWindow.add("checkbox", undefined, undefined, {name: "holdFrame"}); 
            holdFrame.text = "Hold Keyframe"; 
            holdFrame.alignment = ["left","center"]; 

        // ROTATIONREPEATERWINDOW
        // ======================
        var applyBttn = keyFrameRepeaterWindow.add("button", undefined, undefined, {name: "applyBttn"}); 
            applyBttn.text = "Apply | 应用"; 
            applyBttn.alignment = ["center","center"]; 

        keyFrameRepeaterWindow.layout.layout(true);
        keyFrameRepeaterWindow.layout.resize();
        keyFrameRepeaterWindow.onResizing = keyFrameRepeaterWindow.onResize = function () { this.layout.resize(); }

        // ITEM REFERENCE LIST ( Info: http://jongware.mit.edu/Sui/index_1.html ) 
        keyFrameRepeaterWindow.items = { 
        RotationRepeaterWindow: keyFrameRepeaterWindow, // palette
        key1: key1, // group
        ...
        keyFrameRepeaterWindow.itemsArray = [keyFrameRepeaterWindow, keyFrameRepeaterWindow, key1, key1Val, key2, key2Val, freq, freq1, holdFrame, applyBttn];
        ...
        applyBttn.onClick = function(){
            keyFrameRepe(val1, val2, frequency, holdFrame.value);
        }

        if ( keyFrameRepeaterWindow instanceof Window ) keyFrameRepeaterWindow.show();
    
    //Key Frame Repeater function
    function keyFrameRepe(value1, value2, frequency, keyStatusIsHold){
        ...
        try {
            ...
            for(var i=0; i < selected.length; i++){
                ...
                }else if(keyStatusIsHold == false){
                    selected[i].setInterpolationTypeAtKey(key1, KeyframeInterpolationType.LINEAR);
                    selected[i].setInterpolationTypeAtKey(key2, KeyframeInterpolationType.LINEAR);
                    selected[i].setInterpolationTypeAtKey(key3, KeyframeInterpolationType.LINEAR);
                }
            ...
            for(var i=0; i < selected.length; i++){
                ...
                var key1 = selected[i].addKey(time);
                selected[i].setValueAtKey(key1, [value1, value1]);

                var key2 = selected[i].addKey(time+timeFrequency);
                selected[i].setValueAtKey(key2, [value2, value2]);

                var key3 = selected[i].addKey(time+timeFrequency*2);
                selected[i].setValueAtKey(key3, [value1, value1]);
